...
ipm <- nimbleCode({
  ...
  for (t in 1:(nyears-1)){
    ...
    N[t+1, 1] <- sum(rbinom(prob = ft[t], n = round(sum(N[t, AFR:AMAX])), size = 1)) # calves
    #N[t+1, 1] <- round(sum(N[t, AFR:AMAX]))*ft[t]
    for (a in 2:AMAX){
      N[t+1, a] <- sum(rbinom(n = N[t, a-1], prob = S[a-1], size = 1)) # juv and adult 
  ...
  for (t in 1:ncryears){
   #NCalves[t] ~ dbin(PCap, sum(N[cryears[t], 1:2]))
