...
id <- "D50_LCP_IdealWCalves_2023-08-04"
nsim <- 10

folder <- paste0("./Figures/", id)
load(paste0("./Results/ProcResultsMEAN_", id, ".RData"))
             ...
             #   geom_ribbon(data = rtest, aes(x=Year, ymin = KLCI, ymax = KUCI), fill = "lightblue")+
              ...
              #  geom_errorbar(data = ltest, aes(x=Year, ymin = LCI, ymax = UCI, group = Iter))+
               # geom_ribbon(data = rtest, aes(x=Year, ymin = LCI, ymax = UCI), fill = "orange2", alpha = 0.5) +
                geom_line(data = mest, aes(x=Year, y = Mean, group = Iter), color = "orange2") +

    geom_line(data = rtest, aes(x=Year, y = Ntot, group = Iter), color = "blue") +
...
# 
  subplot <- ggplot()+
    ...
    geom_line(data = mest, aes(x=Year, y = Mean/2), color = "blue") +
