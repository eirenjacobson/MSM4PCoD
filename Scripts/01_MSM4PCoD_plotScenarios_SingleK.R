...
for (i in 1:nsim){
  ...
  #                            which(colnames(qdf)=="97.5%")])
                   # K = qdf[which(rownames(qdf) == "K"), 
                    #              which(colnames(qdf)=="50%")],
                    #KLCI = qdf[which(rownames(qdf)=="K"),
                     #                which(colnames(qdf)=="2.5%")],
                    #KUCI = qdf[which(rownames(qdf)=="K"),
                     #                which(colnames(qdf)=="97.5%")])
...
# Plot at the level of the subpopulation (B only)
  ...
  s <- pardf %>% filter(Iter == i) %>%
    pivot_longer(cols = c(K, S2, PCap), names_to = "Parameter", values_to = "Value")
