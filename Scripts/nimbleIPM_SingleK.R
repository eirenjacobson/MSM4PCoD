...
ipm <- nimbleCode({
  ...
  K_scalar ~ dunif(0, 1)
  PCap ~ dunif(0, 1)
  K <- K_lower + K_scalar*(K_upper-K_lower)
  # Process model
  ...
  for (t in 1:(nyears-1)){
    ft[t] <- max(0, f0 + (fmax-f0)*(1-(sum(N[t, 2:AMAX])/K)^z)) # fec at t
  ...
  for (t in 1:nltyears){
    ...
    ltestN[t] ~ dlnorm(mu_log_lt[t], sigma_log_lt[t])
  }
  ...
  for (t in 1:npamyears){
    ...
    pamestN[t] ~ dlnorm(mu_log_pam[t], sigma_log_pam[t])
  }
